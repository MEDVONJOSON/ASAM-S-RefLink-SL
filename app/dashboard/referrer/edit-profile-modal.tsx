...
export function EditProfileModal({ user }: { user: SafeUser }) {
  ...
  const [registeredReferrerCode, setRegisteredReferrerCode] = useState(user.registeredReferrerCode || "")
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setLoading(true)
    try {
      const res = await fetch("/api/referrer/profile", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name, phone, email, imageUrl, registeredReferrerCode: registeredReferrerCode || undefined }),
      })
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>Update your personal and payout information.</DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={onSubmit} className="mt-4">
          ...
          <FieldGroup>
            ...
            <Field>
              <FieldLabel htmlFor="edit-asam-code">ASAM&apos;S Registration Code</FieldLabel>
              <Input
                id="edit-asam-code"
