...
export function EditProfileModal({ user }: { user: SafeUser }) {
  ...
  const [businessCode, setBusinessCode] = useState(user.registeredReferrerCode || "")
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/businesses/profile", {
        ...
        body: JSON.stringify({ name, phone, email, imageUrl, businessCode: businessCode || undefined }),
      })
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button variant="outline" size="sm">Edit Personal Profile</Button>
      </DialogTrigger>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Edit Personal Profile</DialogTitle>
          <DialogDescription>Update your personal owner information. Business details can be edited in Settings.</DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={onSubmit} className="mt-4">
          <div className="mb-6 flex flex-col items-center gap-3">
            ...
            <div>
              <input
                ...
                accept="image/*"
                className="hidden"
                id="personal-profile-pic"
                onChange={handleFileChange}
              />
              <label
                htmlFor="personal-profile-pic"
          ...
          <FieldGroup>
            ...
            <Field>
              <FieldLabel htmlFor="edit-business-code">ASAM&apos;S Authorization Code</FieldLabel>
              <Input
                id="edit-business-code"
                value={businessCode}
                onChange={(e) => setBusinessCode(e.target.value)}
                placeholder="Enter your ASAM'S business code"
              />
            </Field>
          </FieldGroup>
