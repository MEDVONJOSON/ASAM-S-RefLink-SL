  ...
  return (
    <>
      ...
      <div className="fixed bottom-5 right-5 z-50 flex flex-col items-end gap-3">
          ...
          <Card className="w-[min(92vw,380px)] glass-card border-white/10 shadow-2xl shadow-primary/10 p-0 overflow-hidden">
            <CardHeader className="flex flex-row items-center justify-between gap-2 border-b border-border/50 bg-primary/5 py-3 px-4">
              <CardTitle className="flex items-center gap-2 text-sm font-semibold">
                <Sparkles className="h-4 w-4 text-primary" /> ASAMS Guide
              </CardTitle>
        ...
        <Button
          size="icon"
          className="h-16 w-16 rounded-full shadow-2xl shadow-primary/30 hover:scale-105 transition-transform p-0 overflow-hidden border-2 border-primary/20 bg-background/50 backdrop-blur-md"
          onClick={() => setOpen((v) => !v)}
        ...
        >
          {open ? (
            ...
            <img 
              src="/asam-bot.png" 
              alt="ASAM Robot" 
              className="h-full w-full object-cover animate-floating"
            />
          )}
        </Button>
      </div>
